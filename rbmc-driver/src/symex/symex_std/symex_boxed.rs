...
impl<'cfg> Symex<'cfg> {
    ...
    pub(super) fn symex_box_new(&mut self, dest: Expr, args: Vec<Expr>) {
        let lhs = dest.clone();
    ...
    pub(super) fn symex_box_from_raw(&mut self, ret_box: Expr) {
    ...
    pub(super) fn symex_box_into_raw(&mut self, ret: Expr) {
        assert!(ret.ty().is_ptr());
